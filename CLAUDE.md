...
- **IP**: 210.131.208.243
- **ユーザー**: root
- **SSH**: `ssh root@210.131.208.243`
- **SCP**: `scp ファイル root@210.131.208.243:/opt/dlogic/linebot/`
...
# 1. ローカルで編集
# 2. VPSへファイル転送
scp config.py root@210.131.208.243:/opt/dlogic/linebot/config.py
scp bot/line_handlers.py root@210.131.208.243:/opt/dlogic/linebot/bot/line_handlers.py

# 3. サービス再起動
ssh root@210.131.208.243 "systemctl restart dlogic-linebot"

# 4. ログ確認
ssh root@210.131.208.243 "journalctl -u dlogic-linebot --since '1 min ago' --no-pager"
```
